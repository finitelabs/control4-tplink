...
T.section("get_sysinfo -> get_device_info")
...
T.eq("get_sysinfo sends get_device_info", Select(sent, "method"), "get_device_info")
T.check(
...
T.eq("get_sysinfo maps err_code", Select(sysinfo, "err_code"), 0)
T.eq("get_sysinfo decodes nickname", Select(sysinfo, "alias"), "Fountain Pump")
T.eq("get_sysinfo maps model", Select(sysinfo, "model"), "EP25")
T.eq("get_sysinfo maps firmware", Select(sysinfo, "sw_ver"), "1.0.3 Build 240621")
T.eq("get_sysinfo maps rssi", Select(sysinfo, "rssi"), -52)
T.eq("get_sysinfo maps relay_state on", Select(sysinfo, "relay_state"), 1)
...
T.eq("get_sysinfo maps relay_state off", Select(sysinfo, "relay_state"), 0)
T.eq("get_sysinfo tolerates missing nickname", Select(sysinfo, "alias"), nil)

klap:reply({ resolve = { error_code = -1501 } })
outcome = settle(smart:request({ system = { get_sysinfo = {} } }))
T.eq("get_sysinfo maps SMART error", Select(outcome.resolved, "system", "get_sysinfo", "err_code"), -1501)

klap:reply({ reject = { error = "Klap: request failed", code = 500 } })
outcome = settle(smart:request({ system = { get_sysinfo = {} } }))
T.check("get_sysinfo propagates transport failure", Select(outcome.rejected, "error") ~= nil)

T.section("set_relay_state -> set_device_info")
...
T.eq("set_relay_state sends set_device_info", Select(sent, "method"), "set_device_info")
T.eq("set_relay_state on maps device_on=true", Select(sent, "params", "device_on"), true)
T.eq("set_relay_state maps ok", Select(outcome.resolved, "system", "set_relay_state", "err_code"), 0)

klap:reply({ resolve = { error_code = 0 } })
settle(smart:request({ system = { set_relay_state = { state = 0 } } }))
sent = klap.requests[#klap.requests]
T.eq("set_relay_state off maps device_on=false", Select(sent, "params", "device_on"), false)

klap:reply({ resolve = { error_code = 9999 } })
outcome = settle(smart:request({ system = { set_relay_state = { state = 1 } } }))
T.eq("set_relay_state maps SMART error", Select(outcome.resolved, "system", "set_relay_state", "err_code"), 9999)

T.section("emeter.get_realtime -> get_energy_usage")
...
sent = klap.requests[#klap.requests]
T.eq("get_realtime sends get_energy_usage", Select(sent, "method"), "get_energy_usage")
local realtime = Select(outcome.resolved, "emeter", "get_realtime")
T.eq("get_realtime maps ok", Select(realtime, "err_code"), 0)
T.eq("get_realtime maps current_power to power_mw", Select(realtime, "power_mw"), 3251)
...
sysinfo = Select(outcome.resolved, "system", "get_sysinfo")
T.eq("single-outlet sysinfo ok despite child probe rejection", Select(sysinfo, "err_code"), 0)
T.eq("single-outlet has no children", Select(sysinfo, "children"), nil)

local requestsBefore = #klap.requests
klap:reply({ resolve = { error_code = 0, result = { model = "EP25", device_on = true } } })
outcome = settle(single:request({ system = { get_sysinfo = {} } }))
T.eq("single-outlet child probe not repeated", #klap.requests, requestsBefore + 1)
T.eq("single-outlet second sysinfo ok", Select(outcome.resolved, "system", "get_sysinfo", "err_code"), 0)

T.section("Children: multi-outlet devices (SMART power strips)")
...
T.check("strip sysinfo ok", Select(sysinfo, "err_code") == 0, Select(outcome.rejected, "error"))
T.check("strip children paginated", type(children) == "table" and #children == 3)
T.eq("strip child id mapped", Select(children, 1, "id"), "child-1")
T.eq("strip child alias decoded", Select(children, 2, "alias"), "Middle")
T.eq("strip child state on", Select(children, 1, "state"), 1)
T.eq("strip child state off", Select(children, 2, "state"), 0)
T.eq("strip pagination requested next page", Select(klap.requests[#klap.requests], "params", "start_index"), 2)
...
outcome = settle(strip:request({ system = { get_sysinfo = {} } }))
T.check(
  "strip child fetch failure surfaces err_code",
  Select(outcome.resolved, "system", "get_sysinfo", "err_code") ~= 0
)

T.section("Children: control via control_child")
...
sent = klap.requests[#klap.requests]
T.eq("child relay uses control_child", Select(sent, "method"), "control_child")
T.eq("child relay targets device_id", Select(sent, "params", "device_id"), "child-2")
T.eq("child relay nests multipleRequest", Select(sent, "params", "requestData", "method"), "multipleRequest")
local nested = Select(sent, "params", "requestData", "params", "requests", 1)
T.eq("child relay nests set_device_info", Select(nested, "method"), "set_device_info")
T.eq("child relay maps device_on", Select(nested, "params", "device_on"), true)
T.eq("child relay ok mapped", Select(outcome.resolved, "system", "set_relay_state", "err_code"), 0)
...
T.eq("child relay inner error mapped", Select(outcome.resolved, "system", "set_relay_state", "err_code"), -1501)
...
T.eq("child energy nests get_energy_usage", Select(nested, "method"), "get_energy_usage")
T.eq("child energy omits empty params", Select(nested, "params"), nil)
T.eq("child energy maps power_mw", Select(outcome.resolved, "emeter", "get_realtime", "power_mw"), 512)

T.section("Unsupported requests")

outcome = settle(smart:request({ system = { set_led_off = { off = 1 } } }))
T.check("unknown requests are rejected", Select(outcome.rejected, "error") ~= nil)
